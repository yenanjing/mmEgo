...
class Model(nn.Module):
    ...
    def __init__(self, in_channels, hidden_dim, graph_args={},
        ...
        # self.fcn = nn.Conv2d(256, 128, kernel_size=1)
        self.fcn = nn.Conv2d(128, hidden_dim, kernel_size=1)

    def extract_feature(self, x):
        """
        Input:
            x: [batch_size, xyz:3, length_size, joint_num:15, person_num:1]
        Return:
            x: [batch_size, length_size, joint_num, 128]
        """
        B, C, T, V, M = x.size()  # (batch, xyz, length, joint_num, person_num)
