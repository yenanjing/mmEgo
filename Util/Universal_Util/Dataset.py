...
class PosePC(Dataset):
    ...
    def dataRead(self):
        ...
        rootdir = "E:/preprocessed_mmEgo_4_5"
        dirlist = sorted(os.listdir(rootdir))  # 列出文件夹下所有的目录与文件
        orientation_ref = []
        R_ref = []
        for i in range(0, len(dirlist)):
            subDir = os.path.join(rootdir, dirlist[i])
            subDirlist = sorted(os.listdir(subDir), key=lambda x: int(x))
            body_length = []
            st = 0
            for act in range(0, len(subDirlist)):
                subsubDir = os.path.join(subDir, subDirlist[act])
                ...
                for j in range(0, no_per_action):  # 遍历每一次采集的数据
                    ...
                    while len(list_person_once_ti) >= self.frame_no:
                        ...
                        list_all_label.append(dirlist[i])
